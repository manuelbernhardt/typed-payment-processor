...
class ConfigurationSpec extends ScalaTestWithActorTestKit with WordSpecLike {
  "The Configuration actor" should {
    "not find a configuration for an unknown merchant" in {
      ...
      configurationActor ! Configuration.RetrieveConfiguration(
    ...
    "find a merchant configuration for a known merchant and user" in {
      ...
      configurationActor ! Configuration.StoreMerchantConfiguration(
        merchantId,
        MerchantConfiguration(merchantId, BankIdentifier("id")),
        probe.ref)
      probe.expectMessageType[Configuration.MerchantConfigurationStored]

      configurationActor ! Configuration.StoreUserConfiguration(
        UserId("knownUser"),
        UserConfiguration(CreditCard(CreditCardId("id"))),
        probe.ref)
      probe.expectMessageType[Configuration.UserConfigurationStored]

      configurationActor ! Configuration.RetrieveConfiguration(
        MerchantId("knownMerchant"),
        UserId("knownUser"),
        probe.ref)
